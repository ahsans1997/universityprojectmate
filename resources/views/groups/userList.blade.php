                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @forelse ($groupUsers as $groupUser)
                                <tr>
                                    <td class="text-center">{{ $groupUser->user->firstname }}
                                        {{ $groupUser->user->lastname }}</td>
                                        ...
                                        <div class="btn-group" role="group" aria-label="Basic example">
                                            @if (Auth::user()->id == $group->user_id)
                                                ...
                                                <form id="deleteForm-{{ $groupUser->id }}"
                                                    action="{{ route('userGroup.destroy', $groupUser->id) }}"
                                                    method="POST" style="display:none;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <input type="hidden" name="user_id" value="{{ $groupUser->user_id }}">
                                                    <input type="hidden" name="group_id"
                                                        value="{{ $groupUser->group_id }}">
                                                </form>
                                            @endif
                                            <a href="{{ route('users.show', $groupUser->user_id) }}"
